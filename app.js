...
function initPlayersCoverflow() {
    ...
    // Card Clicks & Hover Navigation
    cards.forEach((card, idx) => {
        // Click to bring to center (or allow social link clicks on active center card)
        card.addEventListener("click", (e) => {
        ...
        // Hover / Cursor brings card to center smoothly on desktop
...
/* ==========================================================================
   HERO TITLE — MESH TEXT HOVER WITH CHROMATIC ABERRATION & WARP
   ========================================================================== */
function initMeshTextHover() {
    const title = document.getElementById("heroMeshTitle");
    if (!title) return;

    const displacement = document.getElementById("meshDisplacement");
    const turbulence = document.getElementById("meshTurbulence");
    ...
    // Create Mesh Character DOM hierarchy
    const charData = [];

    lines.forEach((lineText, lIdx) => {
        const lineSpan = document.createElement("span");
        lineSpan.className = "mesh-line";
        lineSpan.setAttribute("data-text", lineText);

        const words = lineText.split(" ");
        words.forEach((wordText, wIdx) => {
            const wordSpan = document.createElement("span");
            wordSpan.className = "mesh-word";
            wordSpan.setAttribute("data-word", wordText);

            for (let char of wordText) {
                const charSpan = document.createElement("span");
                charSpan.className = "mesh-char";
            ...
            if (wIdx < words.length - 1) {
                ...
                spaceSpan.className = "mesh-space";
                spaceSpan.innerHTML = "&nbsp;";
    ...
    // Create Glitch Slice Layers
    const redLayer = document.createElement("div");
    redLayer.className = "mesh-glitch-layer layer-red";
    redLayer.setAttribute("aria-hidden", "true");
    redLayer.innerHTML = `<span>TOTAL GAMING</span><br/><span>ESPORTS</span>`;
    title.appendChild(redLayer);

    const cyanLayer = document.createElement("div");
    cyanLayer.className = "mesh-glitch-layer layer-cyan";
    cyanLayer.setAttribute("aria-hidden", "true");
    cyanLayer.innerHTML = `<span>TOTAL GAMING</span><br/><span>ESPORTS</span>`;
    ...
    let cachedPositions = getCharPositions();
    window.addEventListener("resize", () => {
        cachedPositions = getCharPositions();
    });
    ...
    const updatePhysics = () => {
        ...
        // Animate SVG displacement map scale
        const dispDiff = targetDispScale - curDispScale;
        if (Math.abs(dispDiff) > 0.05) {
            curDispScale += dispDiff * 0.18;
            if (displacement) {
                displacement.setAttribute("scale", curDispScale.toFixed(2));
            }
            isMoving = true;
        } else {
            curDispScale = targetDispScale;
            if (displacement) {
                displacement.setAttribute("scale", curDispScale.toFixed(2));
            }
        }

        if (mouseActive && turbulence && Math.random() < 0.18) {
            turbSeed = (turbSeed % 20) + 1;
            turbulence.setAttribute("seed", turbSeed.toString());
        }

        cachedPositions.forEach(({ data, cx, cy }) => {
            if (mouseActive) {
                const dist = Math.hypot(mouseX - cx, mouseY - cy);
                if (dist < radius) {
                    const factor = Math.pow(1 - dist / radius, 1.8);
                    const angle = Math.atan2(mouseY - cy, mouseX - cx);
                    const force = factor * (window.innerWidth <= 768 ? 22 : 36) + mouseSpeed * 0.2;

                    data.targetX = -Math.cos(angle) * force;
                    data.targetY = -Math.sin(angle) * force * 0.75;
                    data.targetRot = -Math.sin(angle) * factor * 18;
                    data.targetSkew = Math.cos(angle) * factor * 14;
                    data.targetScale = 1 + factor * 0.18;
                    data.targetRgb = factor * (7 + Math.min(mouseSpeed * 0.35, 9));
                } else {
                    data.targetX = 0;
                    data.targetY = 0;
                    data.targetRot = 0;
            ...
            const dx = (data.targetX - data.curX) * 0.2;
            const dy = (data.targetY - data.curY) * 0.2;
            const drot = (data.targetRot - data.curRot) * 0.2;
            const dskew = (data.targetSkew - data.curSkew) * 0.2;
            const dscale = (data.targetScale - data.curScale) * 0.2;
            const drgb = (data.targetRgb - data.curRgb) * 0.22;
            ...
            data.curRot += drot;
            data.curSkew += dskew;
            data.curScale += dscale;
            data.curRgb += drgb;

            // Apply transform to character
            data.el.style.transform = `translate3d(${data.curX.toFixed(2)}px, ${data.curY.toFixed(2)}px, 0) rotate(${data.curRot.toFixed(2)}deg) skewX(${data.curSkew.toFixed(2)}deg) scale(${data.curScale.toFixed(3)})`;

            // Apply Chromatic Aberration RGB text shadow
            if (data.curRgb > 0.3) {
                const rgbOff = data.curRgb.toFixed(1);
                data.el.style.textShadow = `${rgbOff}px 0 0 rgba(255, 0, 85, 0.85), -${rgbOff}px 0 0 rgba(0, 234, 255, 0.85), 0 0 16px rgba(0, 234, 255, 0.5)`;
            } else {
    ...
    const onPointerMove = (clientX, clientY) => {
        ...
        mouseSpeed = Math.min(30, Math.hypot(clientX - prevMouseX, clientY - prevMouseY));
        prevMouseX = clientX;
        prevMouseY = clientY;

        mouseX = clientX;
        mouseY = clientY;
        targetDispScale = Math.min(22, 10 + mouseSpeed * 0.6);
    ...
    const heroSection = document.getElementById("home") || title;
